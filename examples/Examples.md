...
- More data is always better. Especially because we can simulate easily more and more data synthetically. But take care, as more data also means it will take more time to converge. In principle, memory is not a problem as the code is implemented in hdf5 format. Thus you have unlimited memory. On the other hand; if you are using less data than what is needed, your train data set accuracy will diverge from test data set accuracy very early on (<50 epochs). This suggests your training might be improved by using more data to train. As a rule of thumb, we found that 20k simulations for classification and 50k simulations for parameter estimation per model is generally enough when using site frequency spectrum as a summary statistics. 
