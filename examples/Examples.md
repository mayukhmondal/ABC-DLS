...
## Good Practices
- Never believe in one model of neural network. Although I have implemented one neural network model by default, it is always better to run several different neural models (you can find some in [src/extras/](src/extras/) folder) with several separate simulation run and see it reaches the same outcome. After training also use differently masked vcf to ss (different observed data) file gives same results. One example might be to use no mask and mapability mask (using [snapable programme](http://lh3lh3.users.sourceforge.net/snpable.shtml)) ss files and compare the results.    
...
- Remember garbage in garbage out principle. If you use nonsense data as an input, you will get nonsense data as an output. Although using ABC method, it is easier now to catch such situation but by no means it is full proof. On top of it Neural network is black box thus some time it is near impossible to catch such mistake (neural network will learn anything if you force it to learn). Start from already known and accepted results. See if you get similar results and then try to make more complex models on top of it.
