...
To define the observed csv file. Here we put YRI_FRN_HAN (Yoruba, French and Han Chinese) from the [High Coverage HGDP data](https://doi.org/10.1126/science.aay5012) SFS file as SS. It can have multiple lines. Individual line signifies same summary statistics coming from different individuals. Having header is mandatory. If have many lines, every line will run independently for abc analysis. Unlike for parameter estimation or SMC, where an average of the predicted values will be used.  
- --frac  4.636757528
